...
# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='clustering3.log')
...
class MemoryOptimizedParagraphClusterer:
    ...
    def _process_single_file(self, file_path: Path, id_to_index: Dict[str, int]) -> List[Dict[str, Any]]:
        """
        Processes a single text file to extract paragraph metadata.
        
        NOTE: This helper function must be at the top level for the ProcessPoolExecutor 
        to work correctly on all operating systems (e.g., Windows).
        """
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                # Efficiently read the whole file and split by blank lines.
                content = f.read()
                paragraphs = [p.strip() for p in content.split('\n\n') if p.strip()]

            for para_index, para_text in enumerate(paragraphs):
                para_id = f"{paper_id}_para_{para_index:04d}"
                
                # Check if this paragraph has a corresponding embedding
                if para_id in id_to_index:
    ...
    def load_metadata_only_optimized(self) -> pd.DataFrame:
        """
        Load only paragraph metadata without full text, optimized for speed using parallelism.
        
        Args:
            config (Dict[str, str]): A dictionary containing configuration paths,
                                    expected to have 'id_to_index_file' and 
                                    'papers_directory' keys.
        
        Returns:
            pd.DataFrame: A sorted DataFrame containing the paragraph metadata.
        """
        logger.info("Loading metadata only using optimized parallel method...")

        config = self.config
        
        # 1. Load shared data
        try:
            with open(config['id_to_index_file'], 'r') as f:
                id_to_index = json.load(f)
        except FileNotFoundError:
            logger.error(f"ID to index file not found at: {config['id_to_index_file']}")
            return pd.DataFrame() # Return empty DataFrame if critical file is missing

        papers_dir = Path(config['papers_directory'])
        if not papers_dir.is_dir():
            logger.error(f"Papers directory not found at: {papers_dir}")
            return pd.DataFrame()
            
        txt_files = list(papers_dir.glob("*.txt"))
        if not txt_files:
            logger.warning(f"No .txt files found in directory: {papers_dir}")
            return pd.DataFrame()
        ...
        with ProcessPoolExecutor() as executor:
            ...
            future_to_file = {executor.submit(self._process_single_file, f, id_to_index): f for f in txt_files}
        ...
        if not all_metadata:
            logger.warning("No metadata was loaded. Check if paragraph IDs in files match the index file.")
            return pd.DataFrame()

        df = pd.DataFrame(all_metadata)
        df = df.sort_values('embedding_index').reset_index(drop=True)
        
        logger.info(f"Loaded metadata for {len(df):,} paragraphs from {df['paper_id'].nunique()} papers")
        return df
    ...
    def run_memory_optimized_pipeline(self):
        ...
        self.df = self.load_metadata_only_optimized()
        check_memory_usage()
...
def create_memory_optimized_config():
    ...
    return {
        ...
        'max_workers': min(8, cpu_count()),  # Parallel processing workers
        'use_threading': True,  # Use threading (True) vs multiprocessing (False) for I/O
...
def main():
    ...
    parser.add_argument('--max-workers', type=int, default=min(8, cpu_count()), help='Number of parallel workers')
