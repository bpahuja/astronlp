...
def hparam_search(base_model: str, pairs_dir: Path, out_root: Path, lexicon_json: Optional[str],
    ...
    for i, (atype, rf, lr, mprob, bs, ga) in enumerate(combos, 1):
        ...
        pos, rnd = eval_cosine(st, val_path)
        gap = pos - rnd
        results.append({"adapter_type": atype, "reduction": rf, "lr": lr, "mask_prob": mprob, "batch_size": bs, "grad_accum": ga, "gap": gap, "pos": pos, "rand": rnd})
    ...
    print("\\n=== Hyperparam search results (top by gap) ===") ; print(best)
    return best
