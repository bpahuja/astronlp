...
Example (bake-off both bases):
  python astro_methods_bakeoff.py \
    ...
    --device auto --hf_cache /scratch/hf_cache
...
Hyperparam search (on the winning base later):
  python astro_methods_bakeoff.py \
    ...
    --lexicon_json /path/to/lexicon_v2.json \
    --search_trials 6 --search_epochs 2 --hf_cache /scratch/hf_cache
"""
import os, re, json, random, argparse, itertools
...
def build_dataloaders(pairs_dir: Path, tokenizer, batch_size: int, grad_accum: int,
    ...
    per_step_bs = batch_size  # do not divide by grad_accum; we handle accumulation in train_contrastive()
...
def run_single_training(base_id: str, pairs_dir: Path, out_root: Path, lexicon_json: Optional[str], hf_cache: Optional[str], local_files_only: bool,
    ...
    st = build_st_with_adapter(base_id, adapter_name, adapter_type, reduction_factor, max_seq_length, device, hf_cache=hf_cache, local_files_only=local_files_only)
...
def hparam_search(base_model: str, pairs_dir: Path, out_root: Path, lexicon_json: Optional[str],
    ...
    for i, (atype, rf, lr, mprob, bs, ga) in enumerate(combos, 1):
        ...
        st = build_st_with_adapter(base_model, adapter_name, atype, rf, max_seq_length, device, hf_cache=hf_cache, local_files_only=local_files_only)
