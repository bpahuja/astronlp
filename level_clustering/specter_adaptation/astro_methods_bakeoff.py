...
- Optional small hyperparameter search on a single base model (few trials, short epochs).

Requirements:
  pip install "sentence-transformers>=2.7.0" "transformers>=4.40" adapters torch \
              umap-learn hdbscan scikit-learn tqdm pandas

Example (bake-off both bases):
  python astro_methods_bakeoff.py \
    --pairs_dir /path/to/pairs \
    --out_root ./bakeoff_out \
    --bases allenai/specter2_base,adsabs/astroBERT \
    --lexicon_json /path/to/lexicon_v2.json \
    --epochs 6 --batch_size 128 --grad_accum 2 --mask_prob 0.5 \
    --device auto

Optional clustering (sample 50 files):
  python astro_methods_bakeoff.py \
    --pairs_dir /path/to/pairs --out_root ./bakeoff_out \
    --bases allenai/specter2_base,adsabs/astroBERT \
    --lexicon_json /path/to/lexicon_v2.json \
    --paras_dir /path/to/methodology_txts --cluster_sample 50 \
    --umap_dim 30 --n_neighbors 150 --min_cluster_size 10 --min_samples 2

Hyperparam search (on the winning base later):
  python astro_methods_bakeoff.py \
    --mode hparam_search --base_model allenai/specter2_base \
    --pairs_dir /path/to/pairs --out_root ./search_out \
    --lexicon_json /path/to/lexicon_v2.json \
    --search_trials 6 --search_epochs 2
"""
import os, re, json, math, random, argparse, itertools, time
...
def mask_text(text: str, pat_list: List[re.Pattern], mask_prob: float, mask_token: str = "[MASK]", max_spans: int = 5) -> str:
    """
    With probability mask_prob, replace up to max_spans lexicon matches with [MASK].
    Simple string-level masking (not token-aware) but good enough for augmentation.
    """
...
class WindowedPairsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        it = self.data[idx]
        ta = it.a; tb = it.b
        # keyword-masking augmentation (independent for A/B)
...
def build_dataloaders(pairs_dir: Path, tokenizer, batch_size: int, grad_accum: int,
    ...
    per_step_bs = max(1, batch_size // max(1, grad_accum))
...
def build_st_with_adapter(base_id: str, adapter_name: str, adapter_type: str, reduction_factor: int,
    ...
    """
    Build a SentenceTransformer (Transformer+Pooling), attach an adapter (pfeiffer/houlsby),
    set it trainable+active.
    """
    ...
    try:
        ...
        # Fallback: try to add; ignore if already exists
        try:
            word.auto_model.add_adapter(adapter_name, config=cfg)
        except Exception:
            pass
...
def read_method_paras(txt_dir: Path, sample: int = 0) -> pd.DataFrame:
    ...
    if sample and sample < len(files):
        files = rng.sample(files, sample)  # sample files, not paragraphs
    for p in files:
        pid = p.stem
...
def run_single_training(base_id: str, pairs_dir: Path, out_root: Path, lexicon_json: Optional[str],
...
def bakeoff(bases: List[str], **kwargs):
    ...
    # choose winner by largest gap (or lowest noise if available)
...
def hparam_search(base_model: str, pairs_dir: Path, out_root: Path, lexicon_json: Optional[str],
                  ...
                  window_tokens: int, stride: int, max_seq_length: int):
    """
    Small grid/random search over a few knobs. Evaluates by val cosine gap.
    Each trial trains a fresh adapter for a couple of epochs.
    """
    tokenizer = AutoTokenizer.from_pretrained(base_model, use_fast=True)
    mask_patterns = load_lexicon(lexicon_json)
    use_cuda = device.startswith("cuda")

    # candidate space (keep small)
    ...
    for i, (atype, rf, lr, mprob, bs, ga) in enumerate(combos, 1):
        print(f"\n[search {i}/{len(combos)}] {atype} r={rf} lr={lr} mask_prob={mprob} bs={bs} ga={ga}")
        # loaders
        ...
        pos, rnd = eval_cosine(st, val_path)
        gap = pos - rnd
        results.append({"adapter_type": atype, "reduction": rf, "lr": lr, "mask_prob": mprob,
                        "batch_size": bs, "grad_accum": ga, "gap": gap, "pos": pos, "rand": rnd})

    out_root.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(results).sort_values("gap", ascending=False).to_csv(out_root / "hparam_results.csv", index=False)
    best = max(results, key=lambda x: x["gap"])
    Path(out_root / "best_config.json").write_text(json.dumps(best, indent=2), encoding="utf-8")
    print("\n=== Hyperparam search results (top by gap) ===")
    print(best)
    return best
...
def main():
    ...
    if args.mode == "bakeoff":
        ...
        print(f"[search] Best config saved to {out_root}")
